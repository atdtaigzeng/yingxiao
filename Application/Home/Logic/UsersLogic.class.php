...
//        $type = I('get.type',0);
    ...
    public function get_coupon($user_id,$type =0 ){
        //查询条件
        //    $type = I('get.type',0);
        $where = ' AND (l.order_id = 0 or l.use_time= 0) AND c.use_end_time > '.time(); // 未使用
        if($type == 1){
            //已使用
        ...
        $Page = new Page($count,10);
            ...
            $logs = $this->query($sql);
            foreach($logs as $k=>$v){
                $logs[$k]['left_money'] = sprintf("%.2f",  $v['money'] - $v['coupon_money']);
            }
           // var_export($logs);
           // $logs['left_money'] = $logs['money']-$logs['coupon_money'];
        $return['status'] = 1;
        $return['msg'] = '获取成功';
        $return['result'] = $logs;
        //var_export($return['result']);
        $return['show'] = $Page->show();
        return $return;
    }
/*
...
public function get_coupon1(){
    ...
    public function cancel_order($user_id,$order_id){
        ...
        //有余额支付的情况,优惠券支付情况
        //var_export($order['user_money']);
        //var_export($order['coupon_price']);
        //exit();
        if($order['user_money'] > 0 || $order['integral'] > 0 || $order['coupon_price'] > 0){
            //var_export($order);
            //exit();
            accountLog($user_id,$order['user_money'],$order['integral'],$order['coupon_id'],$order['coupon_price'],"订单取消，退回{$order['user_money']}元,{$order['integral']}积分");
        }
        //exit();
