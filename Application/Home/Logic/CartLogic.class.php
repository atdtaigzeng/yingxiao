...
class CartLogic extends RelationModel
{
    ...
    {       
        ...
        $data = array(                    
                    ...
                    'deposit_money'   => $goods['deposit_money'] //商品定金
        ); 
        
        //error_log(print_r($data, 1), 3, "./goods");
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   return $freight;  
    ...
    public function getCouponMoney($user_id, $coupon_id,$mode)
    {
        ...
        if($couponlist['coupon_money'] == $coupon['money']) {
            $coupon['money'] = $coupon['money'] ? $coupon['money'] : 0;
        }else{
            $coupon['money'] = $coupon['money']-$couponlist['coupon_money'];
            $coupon['money'] = $coupon['money'] ? $coupon['money'] : 0;
        }
     ...
     * @param type $shipping_code 改为购买方式
     * @param type $invoice_title 发票
     * @param type $coupon_id 优惠券id
     * @param type $car_price 各种价格
     * @return type $order_id 返回新增的订单id
     */
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$pay_method,$tip)
    {
        ...
        if( !$shipping = M('Plugin')->where("code = '$shipping_code'")->find() ){
            $shipping_code = '';
            $shipping['name'] = "无物流";
        }
        
        if($pay_method=='totle' || !$pay_method){
            $pay_method = '全款支付';
        }else{
            $pay_method = '定金支付';
        }
        ...
        $data = array(
                ...
                'coupon_price'     =>$car_price['couponFee'],//'商品优惠总额',
        ...
        // 2修改优惠券状态  
        if($coupon_id > 0){
                //获取CouponList中对应的Coupon的ID
            $cid = M('CouponList')->where("id = $coupon_id")->getField('cid');
            $money = M('Coupon')->where("id = $cid")->getField('money');
            //判断优惠总金额是否大于优惠券面值
                if( $data['coupon_price'] >= $money ){
                    $data3['uid'] = $user_id;
                    $data3['order_id'] = $order_id;
                    $data3['use_time'] = time();
                    $data3['coupon_money'] = $money;
                    M('CouponList')->where("id = $coupon_id")->save($data3);
                    //$cid = M('CouponList')->where("id = $coupon_id")->getField('cid');
                    M('Coupon')->where("id = $cid")->setInc('use_num'); // 优惠券的使用数量加一
                }else{
                   //将每个订单的优惠总额存入CouponList表中,读取原有的order_id
                    $orderID= M('CouponList')->where("id = $coupon_id")->getField('order_id');
                    //var_export($orderID);
                    $id_array = explode(',',$orderID);
                    //var_export($id_array);
                    if($id_array[0] == 0){
                        $data3['order_id'] = $order_id;
                    }else{
                        array_push($id_array,$order_id);//加入数组
                        $data3['order_id'] = implode(',',$id_array);
                        //var_export($data3);
                       // exit();
                    }
                        $data3['uid'] = $user_id;
                       // $data3['use_time'] = time();//更新使用时间
                        //读取原有的优惠金额
                        $old_money = M('CouponList')->where("id = $coupon_id")->getField('coupon_money');
                        $data3['coupon_money'] = $old_money +  $data['coupon_price'];
                         if($data3['coupon_money']>= $money){
                             //最后一次添加使用时间
                             $data3['use_time'] = time();
                             M('Coupon')->where("id = $cid")->setInc('use_num'); // 优惠券的使用数量加一
                            }
