...
class PromotionController extends BaseController {
	...
	public function prom_goods_save()
	{
        ...
        }elseif ($data['select'] == 1){//存储多活动
            $data['start_time'] = strtotime($data['start_time']);
            $data['end_time'] = strtotime($data['end_time']);
            $data['group'] = implode(',', $data['group']);
            $array = array('name'=>$data['name'],'type'=>$data['type'],'expression'=>$data['expression'],'start_time'=>$data['start_time'],
                'end_time'=>$data['end_time'],'group'=>$data['group'],'id'=>$data['id'],'description'=>$data['description']);
            $array1 = array('name'=>$data['name'],'type'=>$data['type1'],'expression'=>$data['expression1'],'start_time'=>$data['start_time'],
                'end_time'=>$data['end_time'],'group'=>$data['group'],'id'=>$data['id'],'description'=>$data['description']);
            if ($prom_id) {
                M('prom_goods')->where("id=$prom_id")->save($data);
                $last_id = $prom_id;
                ...
                $last_id = M('prom_goods')->add($array);
                $array1['is_connect'] = $last_id;
                $lastid = M('prom_goods')->add($array1);
                adminLog("管理员添加了商品促销 " . I('name'));
            }
        }
