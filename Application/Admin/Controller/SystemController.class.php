...
class SystemController extends BaseController{
	...
	{          
		...
		$inc_type =  I('get.inc_type','shop_info');//默认为shop_info
     ...
     */        
      ...
      * 抽奖队列
      */
     /* private function prize_line(){
         $users = M('users')->where(1)->field("user_id,first_leader,second_leader,third_leader")->select();
         $length = count($users);
         ...
         for($i=0; $i<$length; $i++){
             ...
             if($users[$i]['third_leader']>0){
                 
                 $uid = (int)$users[$i]['third_leader']; //抽奖用户
                 $first_leader = (int)$users[$i]['second_leader'];
                 $second_leader = (int)$users[$i]['first_leader'];
                 $third_leader = (int)$users[$i]['user_id'];
                 
                 $data = [
                     'uid' => $uid, //抽奖用户，团队最高级
                     'first' => $first_leader,//团队一级
                     'second' => $second_leader,//团队二级
                     'third' => $third_leader,//团队三级
                 ];
                 
                 //同一团队只能进行一次排队
                 if( !$prize_line->where($data)->find() ){
                     
                     if(!$prize_line->where("uid={$uid} and is_deal=0")->find()){
                         //同一人在抽奖队列中只能出现一次
                         $data['addtime'] = time();
                         $prize_line->add($data);
                     }
