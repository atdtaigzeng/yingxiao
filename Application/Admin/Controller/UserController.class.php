...
class UserController extends BaseController {
    ...
    public function detail(){
        ...
        //查询用户的优惠券的使用情况
        $coupon_list = M('coupon_list')->where("uid = ".$uid."")->select();
        $used = M('coupon_list')->where("uid= ".$uid." and use_time != 0")->select();
        $not_use = M('coupon_list')->where("uid = ".$uid." and use_time =0")->select();
       // var_export(I('get.id'));
       // var_export($coupon_list);
        $list['sum'] = count($coupon_list);
        $list['used'] = count($used);
        $list['not_use'] = count($not_use);
        //var_export($list);
...
//         $data = array($status,$user_id,$account_bank,$account_name,$create_time);
//         dump($data);die;
    ...
    public  function editWithdrawals(){
        ...
        $time_status = $withdrawals['time_status'];
...
//             dump($model);die;
            ...
            if($model->status == 1 && $withdrawals['status'] != 1)
            {
                ...
                $trans = new \Admin\Controller\TransController();
//                 $trans->test();

                $trans_money = $withdrawals['money'];
                
                //利息扣除
                if($time_status==1){
                    //t+1
                    $trans_money = $trans_money-$trans_money*0.006;
                }elseif($time_status==3){
                    //t+3
                    $trans_money = $trans_money-$trans_money*0.003;
                }else{
                    $trans_money = $trans_money-5;
                }
                
                $res = $trans->trans($user['openid'],$trans_money);
                ...
                M('remittance')->add($remittance);//转账记录
            }
            $model->save();
