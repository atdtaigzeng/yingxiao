...
class GoodsController extends MobileBaseController {
    ...
    public function search(){
        ...
        if($condition != null) {//关键分类字搜索
            if($condition == '促销'){
                $filter_goods_id = M('flash_sale')->getField("goods_id",true);
            }else {
                $filter_goods_id = M('goods')->where("is_on_sale=1 and ".$condition."")->cache(true)->getField("goods_id", true);
            }
        }else{
