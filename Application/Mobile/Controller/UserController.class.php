...
class UserController extends MobileBaseController
{
    ...
    */
    ...
    public function index()
    {
        ...
        $userinfo = $this->user;
        if(!$userinfo['mobile']){
            echo "<script type='text/javascript'>alert('请先认证手机号！');window.location.href='/index.php/Mobile/User/userinfo';</script>";
            die;
        ...
        /* if($userinfo['first_leader']==0){
            //没有上级
            echo "<script type='text/javascript'>alert('您还没有推荐人，请完善推荐人信息！');window.location.href='/index.php/Mobile/User/userinfo';</script>";
            die;
        } */
...
;        }else{
    ...
    public function userinfo()
    {
        ...
        if (IS_POST) {
            ...
            I('post.leader') ? $leader = I('post.leader') : false; //手机
            ...
            if($leader){
                $length = strlen($leader);
                if($length>10){
                    //手机号
                    if( $leader_info = M("users")->where("mobile='{$leader}'")->field("user_id,is_distribut,first_leader,second_leader")->find() ){
                        
                        if($leader_info['is_distribut']==0){
                            $this->error('该用户还不是分销商!');
                        }else{
                            $post['first_leader'] = $leader_info['user_id'];
                            $post['second_leader'] = $leader_info['first_leader'];
                            $post['third_leader'] = $leader_info['second_leader'];
                        }
                    ...
                    //ID
                    $leader = (int)$leader;
                    if( $leader_info = M("users")->where("user_id={$leader}")->field("user_id,is_distribut,first_leader,second_leader")->find() ){
 ...
 *
 */
public function is_agree(){

    $info = I('post.dateAgree');
    $id = I('post.userid');
    if ($info == 1){
        $data['is_agree'] = $info;
       $res = M('users')->where("user_id=".$id)->save($data);
        if($res !== false){
            echo 1;
        }
    }
    ...
    */
    ...
    {
        $type = I('type');//发送类型,分为mobile,email
        $send = I('send');//接收用户
        
        if($tixian = I('tixian')){
            //判断手机号是否匹配
            $user_info= $this->user;
            $user_num = $user_info['mobile'];
            
            if($user_num != $send){
                $res = array('status'=>-1,'msg'=>'与认证手机号不匹配');
        ...
        //error_log(print_r($res, 1), 3, "./sendmessage");
   ...
   public  function recharge(){
    ...
    public function withdrawals(){
        ...
        $order_count = M('order')->where($where)->count(); // 我的订单数
        $goods_collect_count = M('goods_collect')->where($where)->count(); // 我的商品收藏
        $comment_count = M('comment')->where($where)->count();//  我的评论数
        $this->assign('order_count', $order_count);
        $this->assign('goods_collect_count', $goods_collect_count);
        $this->assign('comment_count', $comment_count);
    ...
    public function check_leader(){
        ...
        if($length>10){
            //手机号
            if( $leader_info = M("users")->where("mobile='{$leader}'")->field("user_id,is_distribut")->find() ){
                
                if($leader_info['is_distribut']==0){
                    //不是分销商
                    $res = array('status'=>-1,'msg'=>'该用户还不是分销商');
                }elseif($leader_info['user_id']==$user_id ){
                    $res = array('status'=>-1,'msg'=>'自己不能成为上级');
                }else{
                    $res = array('status'=>1);
                }
            }else{
                //没有该用户
                $res = array('status'=>-1,'msg'=>'没有该用户！');
            }
            
        }else{
            //通过ID增加上级
            $leader = (int)$leader;
            if( $leader_info = M("users")->where("user_id={$leader}")->field("user_id,is_distribut")->find() ){
                if($leader_info['is_distribut']==0){
                    //不是分销商
                    $res = array('status'=>-1,'msg'=>'该用户还不是分销商');
                }elseif($user_id==$leader){
                    $res = array('status'=>-1,'msg'=>'自己不能成为上级');
                }else{
                    $res = array('status'=>1);
                }
