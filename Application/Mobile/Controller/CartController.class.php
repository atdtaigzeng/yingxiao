...
class CartController extends MobileBaseController {
    ...
    public function cart2()
    {
        ...
        //计算购物车中物品的定金费用
        $length = count($result['cartList']);
        $deposit_money = 0;
        $is_deposit = 0; //区分购买的商品中是否可以定金支付
        $is_weight = 0;
        
        for($i=0; $i<$length; $i++){
            if($result['cartList'][$i]['selected']==1){
                
                $select_goods_id = $result['cartList'][$i]['goods_id'];
                
                if($is_weight==0){
                    $goods_weight = M('goods')->where("goods_id={$select_goods_id}")->getField('weight');
                ...
                //购物车选中状态
                if($result['cartList'][$i]['deposit_money']==0){
                    //没有定金
                    $deposit_money = $deposit_money + $result['cartList'][$i]['goods_price']*$result['cartList'][$i]['goods_num'];
                }else{
                    //有定金
                    $is_deposit = 1;
                    $deposit_money = $deposit_money + $result['cartList'][$i]['deposit_money']*$result['cartList'][$i]['goods_num'];
                }
                
                
            }
        }
        $this->assign('is_deposit',$is_deposit);
        $this->assign('deposit_money',$deposit_money);
        $this->assign('is_weight',$is_weight);
        //dump($deposit_money);die;
        
        
        $shippingList = M('Plugin')->where("`type` = 'shipping' and status = 1")->select();// 物流公司

        $Model = new \Think\Model();
        // 找出这个用户的优惠券 没过期的  并且 订单金额达到 condition 优惠券指定标准的。有两种购物券一种适合未使用的，一种是使用了未使用完的
        /*
         * 下面修改为两种情况
         * (1)用户购买的商品的累积的优惠金额超过优惠券的面值，即该优惠券只使用了一次
         * (2)用户该买的商品的累积金额未超过优惠券的面值，即该优惠券使用多次
         */
        $sql = "select c1.name,c1.money,c1.condition, c2.* from __PREFIX__coupon as c1 inner join __PREFIX__coupon_list as c2  on c2.cid = c1.id and c1.type in(0,1,2,3) and (order_id = 0 or use_time =0 ) where c2.uid = {$this->user_id} and ".time()." < c1.use_end_time and c1.condition <= {$result['total_price']['total_fee']}";
        $couponList = $Model->query($sql);

        //读取未用完的优惠券
       // exit();
        //判断购物车中的商品是否有使用优惠券的商品,有就将优惠券送过去，没有就不送
        $info = $result['cartList'];
        foreach ($info as $k =>$v){
            if($v['selected'] == 1) {
                $arr[$k] = $v['goods_id'];
                $num[$k] = $v['goods_num'];
        ...
        $goods = M('goods');
        foreach ( $arr as $k=>$v){
            $array[$k] = $goods->where("goods_id = '".$v."'")->getField('offset');
            $sum[$k] = $array[$k]*$num[$k];
        }
        $all_sum =sprintf("%.2f",array_sum($sum));
        foreach ($couponList as $k=>$v){
            $couponList[$k]['left_money'] =sprintf("%.2f" , $v['money']-$v['coupon_money']);
        }
          if($all_sum > 0) {
              //算总的剩余抵用额度队列
              $this->assign('sum',$all_sum);
    ...
    public function cart3(){
        ...
        $shipping_code =  I("shipping_code"); //  物流方式
        $pay_method = I("pay_method"); //购买方式
        $tip = I('tip');//备注信息
        ...
        //if(!$shipping_code) exit(json_encode(array('status'=>-4,'msg'=>'请选择物流','result'=>null))); // 返回结果状态
		
		$address = M('UserAddress')->where("address_id = $address_id")->find();
		$order_goods = M('cart')->where("user_id = {$this->user_id} and selected = 1")->select();
        $result = calculate_price($this->user_id,$order_goods,$shipping_code,0,$address[province],$address[city],$address[district],$pay_points,$user_money,$coupon_id,$couponCode,$pay_method);
	...
	// 订单满额优惠活动		                
        ...
        $car_price = array(
            ...
            'buy_method'    => $result['result']['buy_method'],//购买方式
            'couponFee'    => $result['result']['coupon_price'], // 商品优惠总额
        ...
        {  
            ...
            $result = $this->cartLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,$pay_method,$tip); // 添加订单                        
            exit(json_encode($result));            
        }
